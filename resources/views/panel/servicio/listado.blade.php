    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            @foreach($servicios AS $item)
                <tr>
                    <td>{{str_pad($item->idservicio,7,'0000000',STR_PAD_LEFT)}}</td>
                    <td>{{ $item->titulo }} </td>
                    <td>{!! $item->estado ? '<label class="badge badge-success">Habilidado</label>' : '<label class="badge badge-danger">Inhabilitado</label>' !!}</td>
                    <td class="text-center">
                        <div class="dropdown">
                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenu-{{$item->idservicio}}" data-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false">
                                Seleccione
                            </button>
                            <div class="dropdown-menu" aria-labelledby="dropdownMenu-{{$item->idservicio}}" data-idservicio="{{$item->idservicio}}">
        ...
        <p>
            <input type="hidden" name="paginaActual" id="paginaActual" value="{{$servicios->currentPage()}}">
            Mostrando del registro {{$servicios->firstItem()}} al {{$servicios->lastItem()}} de un total de {{$servicios->total()}} registros
        </p>
